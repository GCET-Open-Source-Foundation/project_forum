							...
							(link) =>
								link.visible && (
									<a
										key={link.name}
										href={link.href}
										className="block border-2 border-black bg-white p-3 text-sm font-bold uppercase shadow-[4px_4px_0_#000]"
									>
										{link.name}
									</a>
								),
	...
	return (
		<div className="w-full max-w-md bg-white border-2 border-black shadow-[8px_8px_0_#000] p-8 relative">
				...
				<div>
					...
					<div className="flex gap-2">
						<input
							...
							className="flex-1 bg-zinc-50 border-2 border-black p-3 text-sm disabled:bg-zinc-200"
						/>
						<button
							type="button"
							onClick={handleSendOtp}
							disabled={loading || otpSent}
							className="bg-black text-white px-4 text-xs font-bold border-2 border-black hover:bg-white hover:text-black transition disabled:opacity-50"
						>
			...
			<div className="mt-8 pt-6 border-t-2 border-dashed border-zinc-300 text-center text-xs">
				<span className="text-zinc-500">EXISTING_USER? </span>
				<a
					href="/login"
